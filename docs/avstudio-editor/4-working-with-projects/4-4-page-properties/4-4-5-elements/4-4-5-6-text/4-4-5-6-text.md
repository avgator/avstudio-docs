...
Element Constraints control positioning anchors and stacking order.

### Constraints X Axis

Defines the horizontal anchor point of the element.

- Example: `Left`
- Accepts dynamic values and expressions.

### Constraints Y Axis

Defines the vertical anchor point of the element.
...
Controls the stacking order relative to other elements.

- Higher values render above lower ones
- Supports dynamic values and expressions

## Dimensions

Dimensions group sizing-related behavior and determine how the Text element occupies space.

- Width and Height units: `px` or `%`
- Each field supports `Assign Dynamic Value` and post-processing functions
- Works together with layout constraints to produce the final on-page size
...
Appearance in the panel maps to the styling controls documented below in the `Design` section. It covers color, background, font, border, shadow, and reflection.
...
Note: `Inner Content Gap` is equivalent to the `Icon‑Text Gap` mentioned in other panels.
...
`Actions` define interactive behavior for the Text element.

### Add New Action

Selecting `Add New Action` opens the `Create New Action for Text` modal.

### Create New Action for Text

Configure how the element responds to interaction (tap, click, etc.).

#### General Settings

- **Name**: Action label for identification
- **Behaviour**: How the action is triggered (e.g., `Tap`)
- **Execution Delay**: Delay before execution in milliseconds
...
#### Crestron Action Example

- **Type**: Crestron action type (e.g., `Push Digital`)
- **Signal Name**: The target signal to trigger (required)
- **Push Data**: `True`, `False`, `Pulse`, `Momentary`, or `Toggle`
- **Pulse Delay**: Pulse duration in milliseconds

Click `Create action` to save and attach it to the Text element.
...
Configure how the Text element’s live value is sourced and handled.

Tabs:

- **Direct Value**: Provide the value directly or via a dynamic expression. Use this for local defaults or programmatic updates.
- **Crestron FB**: Bind the value to a Crestron feedback signal so the element reflects controller feedback.

Field: **Value**
- Click `{-}` to assign Variables/Parameters/State; optionally use `fx` for transformation.

Field: **Checked Value**
- Applies when the Text element is configured to reflect a boolean state (e.g., on/off indicator). Hidden when not applicable.

Toggle: **Auto focus on load**
- When enabled, focuses the element automatically when the page loads.
...
Preserves layout space when the element is hidden. Toggle on to avoid layout shifts.
...
- Supports feedback-driven dynamic values and expressions.
