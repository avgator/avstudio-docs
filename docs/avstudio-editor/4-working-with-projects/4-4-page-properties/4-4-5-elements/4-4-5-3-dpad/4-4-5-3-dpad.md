...
- `Default Color`: The base color of the Dpad.
  - Use the color picker or type a color name (e.g., `red`) or hex value (e.g., `#FF0000`).
- `Pressed Color`: The color shown while the Dpad is pressed.
  - Use the color picker or type a color name or hex value.
- `Text Color`: The color of any labels/text rendered with the Dpad.
  - Use the color picker or type a color name or hex value.
  - Supports `Assign dynamic value` `{−}` to drive the color from `Variables` or `Parameter`.

### Font

Control the typography of labels rendered by the Dpad:

- `Font Family`: Choose the typeface for Dpad text.
  - `Default`: Uses the project’s default font family.
  - Use the drop-down to select another available font family.
- `Font Size`: Set how large the text appears.
  - `Assign dynamic value` `{−}`: Opens the `Pick dynamic value` modal to drive the size from `Variables` or `Parameter`.
  - Direct value: Enter a numeric size and select a unit.
  - Unit options: `px`, `em`, `vh`, `vw`.
- `Text Transform`: Adjust the casing of the text.
  - Options: `None`, `Uppercase`, `Lowercase`, `Capitalized`.
